...
if __name__ == "__main__":
    ...
    csv_dir =  '/media/xuchengjun/datasets/action_zx/train'   # /media/xuchengjun/datasets/action_zx
    hy5file_data = '/media/xuchengjun/datasets/action_zx/data_test_ml.h5'   #不能用纯数字的名字 /media/xuchengjun/datasets/UTD-MAD/cs2/data_test.h5
    hy5file_label = '/media/xuchengjun/datasets/action_zx/label_test_ml.h5'
    csv_list = os.listdir(csv_dir)
    random.shuffle(csv_list)
    # embed()

    # csv_list.sort(key=lambda x:int(x.split('.')[0].split('_')[1]))
    f_3 = h5py.File(hy5file_data, 'a')
    f_4 = h5py.File(hy5file_label, 'a')
