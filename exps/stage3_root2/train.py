...
# from model.main_model.new_model import SMAP
...
def main():
    ...
    with Engine(cfg, custom_parser=parser) as engine:
        logger = engine.setup_log(
            name='train', log_dir=cfg.OUTPUT_DIR, file_name='log0328.txt')
        args = engine.args
        ...
        cfg.SOLVER.MAX_ITER = int(cfg.SOLVER.MAX_ITER * 6 / num_gpu)
        ...
        for iteration, (images, valids, labels, rdepth_map, rdepth_mask) in enumerate(data_loader, engine.state.iteration):
            ...
            if iteration % 5000 == 0 and iteration >= 20000:
                engine.update_iteration(iteration) # 这个是为了实时更新iteration的值,用来保存不同的中间步的模型
                # 这个是为了保存每2000步的值
                # save ckpt
                if not engine.distributed or engine.local_rank == 0:
