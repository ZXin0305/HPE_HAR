...
class Config:
    ...
    TENSORBOARD_DIR = Path('/media/xuchengjun/zx/human_pose/pth/main/20220328')  # tensorboard
    ...
    MODEL.GPU_IDS = [1,2]                     # set useful gpu-ids
    MODEL.SAVE_PATH = '/media/xuchengjun/zx/human_pose/pth/main/20220328'                      # the path to save model
    MODEL.CK_PATH = '/media/xuchengjun/zx/human_pose/pth/main/20220328'                        # the path to save/load model checkpoint  
    # /home/xuchengjun/ZXin/human_pose/pretrained/SMAP_model.pth
    # /media/xuchengjun/zx/human_pose/pth/main/20220328/train.pth
    MODEL.WEIGHT = "/home/xuchengjun/ZXin/human_pose/pretrained/SMAP_model.pth"                       # osp.join(ROOT_DIR, 'lib/models/resnet-50_rename.pth') /media/xuchengjun/zx/human_pose/pretrained

    # -------- Training Config -------- #
    #训练参数设置
    SOLVER = edict()
    SOLVER.IMG_PER_GPU = 1                    #训练时，每个ＧＰＵ处理多少图片
    SOLVER.BASE_LR = 2e-4                      #学习率
    SOLVER.CHECKPOINT_PERIOD = 2000            #保存中间的训练参数
    SOLVER.MAX_ITER = 30000 * 2                  #总的ite
    ...
    TEST_DIR = 'main'
    TEST_PATH = Path('/media/xuchengjun/datasets/CMU/refine_json_file')      # path to save the final-json file
